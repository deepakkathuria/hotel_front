    ...
    <div style={pageStyle}>
      ...
      <div style={stepStyle}>
        ...
        <button
          ...
          onClick={() =>
            navigate(
              `/?checkin=${checkin}&checkout=${checkout}&adults=${adults}&children=${children}&edit=true`
            )
          }
        >
          ✏️ Edit
      ...
      <div style={layoutStyle}>
        ...
        <div style={summaryStyle}>
          ...
          <div style={summaryRow}><strong>No. of Nights</strong><span>{nights} Night{nights > 1 ? "s" : ""}</span></div>
          ...
          <hr />
          <div style={summaryRow}><strong>Room only</strong><span>₹{room.base_price} x {nights} = ₹{subtotal}</span></div>
...
const pageStyle = { width: '100vw', padding: '30px 50px', fontFamily: 'Segoe UI, sans-serif', background: '#fff', color: '#222', boxSizing: 'border-box' };
const headerStyle = { fontSize: 24, marginBottom: 10, borderBottom: '1px solid #ccc', paddingBottom: 5 };
const stepStyle = { background: '#f8f8f8', padding: '10px 15px', marginTop: 10, borderLeft: '5px solid #b59a45', fontWeight: 600 };
const stepActiveStyle = { ...stepStyle, background: '#444', color: '#fff' };
const layoutStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 40, marginTop: 30, flexWrap: 'wrap' };
const formBoxStyle = { flex: 1, minWidth: 320 };
const summaryStyle = { flex: 1, minWidth: 320, border: '1px solid #ccc', borderRadius: 6, padding: 20, background: '#f9f9f9' };
const summaryRow = { display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontSize: 14 };
const inputStyle = { width: '100%', padding: 10, marginBottom: 12, border: '1px solid #ccc', borderRadius: 5 };
const submitButton = { width: '100%', padding: 14, background: '#b59a45', color: 'white', fontSize: 16, border: 'none', borderRadius: 6, cursor: 'pointer', marginTop: 10 };
